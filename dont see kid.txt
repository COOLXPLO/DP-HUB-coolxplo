...
-- 🔹 Ensure `Main` and `ui` are defined
local Main = Main or script.Parent:FindFirstChild("Main") -- Replace with actual UI reference
local ui = ui or script.Parent -- Adjust as needed
...
-- 🔹 Small Circle Button (Always Visible)
local CircleButton = Instance.new("TextButton")
CircleButton.Name = "CircleButton"
CircleButton.Parent = ui
CircleButton.Position = UDim2.new(0.05, 0, 0.1, 0) -- Adjust position
CircleButton.Size = UDim2.new(0, 30, 0, 30) -- Small circular size
...
CloseButton.MouseButton1Click:Connect(function()
    Main.Visible = false -- Hide main UI
    CircleButton.Visible = true -- Show circle button
end)

-- 🔹 Circle Button Functionality (Click to Reopen UI instead of Hover)
CircleButton.MouseButton1Click:Connect(function()
    Main.Visible = true -- Show UI when clicked
    CircleButton.Visible = false -- Hide the circle button
end)

-- 🔹 Make the Circle Button Movable
local UserInputService = game:GetService("UserInputService")
...
UserInputService.InputChanged:Connect(function(input)
    if input == dragInput and dragging then
        ...
        -- Keep CircleButton within screen bounds
        local screenSize = game:GetService("Workspace").CurrentCamera.ViewportSize
        local newX = math.clamp(startPos.X.Offset + delta.X, 0, screenSize.X - CircleButton.Size.X.Offset)
        local newY = math.clamp(startPos.Y.Offset + delta.Y, 0, screenSize.Y - CircleButton.Size.Y.Offset)
